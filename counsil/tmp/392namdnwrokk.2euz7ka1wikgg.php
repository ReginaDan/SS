              ...
              <div class="panel panel-visible" id="spy1">
                <div class="panel-heading">
                  <div class="panel-title hidden-xs">
                    <span ></span>Список студсоветов факультетов</div>
                </div>
                <div class="panel-body pn">
                  <table  class="table" cellspacing="0" width="100%">
                    <thead>
                      <tr>
                        <th>Название студсовета</th>
                        <th>Ссылка</th>
                        <th style="width: 40px">Действие</th>
                      </tr>
                    </thead>
                    <tbody>
	                    <?php foreach (($local?:array()) as $loc): ?>
		                    <tr>
			                    <td><?php echo $loc['name']; ?></td>
			                    <td><?php echo $loc['link']; ?></td>
			                    <td>
				                    <a href="<?php echo $BASE; ?>/admin/locals/list/change/<?php echo $loc['id']; ?>" type="button" class="btn btn-xs btn-info btn-block">Изменить</a>
	                                <form method="post" action="<?php echo $BASE; ?>/admin/locals/list/delete/<?php echo $loc['id']; ?>">
	                                <input type="text" name="id" id="id" class="hidden" value="<?php echo $loc['id']; ?>"> 
				  ...
				  <a href="<?php echo $BASE; ?>/admin/locals/list/add" type="button" class="btn btn-sm btn-dark btn-block">Добавить</a>                
				 </div>
              </div>
            </div>
          </div>
